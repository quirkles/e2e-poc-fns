...
export function createPinoLogger(
  ...
  const loggingConfig: PLoggerOptions<LogLevel> = {
    ...
    formatters: {
      ...
      level(level: string): { severity: string; level: Severity } {
  ...
  if (getConfig().env === "local") {
    ...
    transports.push(pinoDestination(1));
  }
...
class PinoLogger extends Logger {
  ...
  child(optionOverrides: LoggerOptions = {}): Logger {
    ...
    const childLogName =
      optionOverrides.logName ?? `${parentName as string}_child`;

    const logMeta = {
      _LOG_META_parentLoggerName: parentName as string,
      _LOG_META_parentLoggerInstanceUuid: parentLoggerInstanceUuid as string,
